...
def lookup_book(books):
    number = 1
    Author = input("please enter the author name you want to serch: ")
    for book in books:
        for key,value in book.items():
            if value == Author:
                print("book_id: {}, book_name: {}, author: {}, book_year: {}, publisher: {}"
                    .format(book['book_id'], book['book_name'], book['author'],
                    book['book_year'], book['publisher']))
                number -= 1
                break
    if number == 1:
        print("您输入的作者不存在，请重新输入！")